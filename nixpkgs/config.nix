{
  ...
  packageOverrides = super:
    let
        ...
        ghcide-nix = (import (githubTarball "cachix" "ghcide-nix" "master") {})."ghcide-${compilerVersion}";
        # nb. The new ghc version may not be supported. Check the repo (link on top)
     in
      {
        myHaskellEnv =
          (self.haskell.packages.${compilerVersion}.ghcWithHoogle
             (haskellPackages: with haskellPackages; [
                 # Libs
                 xmonad-extras xmonad-contrib dbus # Required to compile xmonad (pacman:xmonad install it's own ghc version...)

                 # Tools
                 cabal-install stack # Placing ghcid/ghcide here doesn't work :(
